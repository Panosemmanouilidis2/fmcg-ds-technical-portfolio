...
Deployed on Google Cloud Run. Model loaded directly from GCS at startup.
Architecture: GCS artifact store → XGBoost Booster inference (no idle endpoint cost).
...
# CUSTOMER_LABELS loaded from private config — not committed to repo
from retailer_config import CUSTOMER_LABELS
...
# ══════════════════════════════════════════════════════════════════════════════
# MODEL LOADER — GCS direct (no idle Vertex endpoint cost)
# ══════════════════════════════════════════════════════════════════════════════
GCS_BUCKET  = "unilever-promo-ml-data"
GCS_MODEL   = "models/xgb_uk_tuned.json"
LOCAL_MODEL = "/tmp/xgb_uk_tuned.json"
...
def load_resources():
    ...
    bucket.blob(GCS_MODEL).download_to_filename(LOCAL_MODEL)
...
# PREDICTION
# ══════════════════════════════════════════════════════════════════════════════
def predict(c, booster, feature_names, medians, scaling):
    import xgboost as xgb
    ...
    payload  = np.array([[float(row[col]) for col in feature_names]])
    dmatrix  = xgb.DMatrix(payload, feature_names=feature_names)
    log_pred = float(booster.predict(dmatrix)[0])
    pred_vol = max(float(np.expm1(log_pred)), 0) * 1000  # model trained on log1p(volume/1000)
...
def main():
    ...
    try:
        booster, feature_names, medians, scaling = load_resources()
    ...
    if run:
        ...
        with st.spinner("Running forecast models..."):
            results = [predict(c, booster, feature_names, medians, scaling) for c in campaigns]
